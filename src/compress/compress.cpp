...
// -------------------------------------------------------------------------------------------------------------------
	...
	char compressed_output[file_size][MAX_HUFF_ENCODING_LENGTH + 1];
	ulong prefix_sums[file_size];
...
// -------------------------------------------------------------------------------------------------------------------
	...
	uint gap_array_size = (compressed_file_size_bits / SEGMENT_SIZE) + (compressed_file_size_bits%SEGMENT_SIZE != 0);
	uint prefix_sums_size = file_size;
	uint gap_idx = 1;
	ulong sum = 0, next_limit = 0;
	short gap_array[gap_array_size];

	gap_array[0] = 0;
	next_limit = SEGMENT_SIZE * gap_idx;

	for(uint i=0 ; i<prefix_sums_size-1 ; i++){
		sum += prefix_sums[i];
...
// ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ Huffman Info ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
	...
	for(int i=0 ; i<total_frequencies.size() ; i++){
...
// ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ Gap Array ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
	...
	uint temp_gap_array_size = gap_array_size;
	ushort last;
	...
	for(uint i=0 ; i<gap_array_size ; i++){
		output_header_buffer += to_string(gap_array[i]);
	}
...
// ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ Segment Size ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
	...
	uint temp_segment_size = SEGMENT_SIZE;
...
// ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ Compressed Data ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
	...
	for(ulong i=0 ; i<file_size ; i++){
		...
		while(compressed_output[i][j] != '\0'){
			write_remaining = true;
			next_bit = (int)compressed_output[i][j] - 48;
			ch << 1;
			ch ^= next_bit;
			++counter;
