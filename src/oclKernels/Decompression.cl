__kernel void huffDecompress(__global char *compressed_buffer,
                             ...
                             const int compressed_data_size_bits, const int segment_size,
  ...
  if (global_id == 0) {
    ...
    read_offset = ((global_id - 1) * segment_size)  + segment_size + gap_arr[global_id] + 1;
  }
  int decomp_index = 1;
  ...
  // printf("read_offset = %ld ", read_offset);
  ...
  while (*global_counter != total_segments) {
    if (*global_counter == global_id) {
      ...
      while (i < decoded_length) {
        ...
        // printf("decompressed_buffer[*global_decompressed_offset] : %d",  decompressed_buffer[*global_decompressed_offset]);
        ++i;
